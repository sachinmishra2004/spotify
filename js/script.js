// console.log("lets us start the javascript");
let currentSong = new Audio();
let songs;
let currFolder;
//this function from fetch the songs from the songs folder
...
async function getSongs(folder) {
  ...
  for (let index = 0; index < as.length; index++) {
    ...
    if (element.href.endsWith(".mp3")) {
      songs.push(element.href.split(`/${folder}/`)[1]); // songs ke url main le liya
    }
  }
  //play the first song
...
async function main() {
  ...
  // attch an eventListner to play , next and privoius
  ...
  //add an event humburger
  document.querySelector(".hamburger").addEventListener("click", () => {
    document.querySelector(".left").style.left = "0";
  });

  //add an event crooss button
  ...
  //add event listener to mute the react
